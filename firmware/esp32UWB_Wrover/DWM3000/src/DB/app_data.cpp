...
bool AppDataManager::parseBlePayload(String payload) {
    ...
    // --- PARSOWANIE KOTWIC UWB ---
    String uwbStr = payload.substring(uwbIndex + 2, bleIndex);
    uwb_anchor_count = 0;
    for (int i = 0; i < uwbStr.length() && i < MAX_UWB_ANCHORS; i++) {
        active_uwb_anchors[i] = uwbStr.charAt(i);
        uwb_anchor_count++;
    }

    // --- PARSOWANIE TAGÓW BLE ---
    String bleStr = payload.substring(bleIndex + 3);
...
uint8_t AppDataManager::getUwbAnchorCount() {
    return uwb_anchor_count;
}

char AppDataManager::getUwbAnchorId(uint8_t index) {
    if (index < uwb_anchor_count) return active_uwb_anchors[index];
    return '\0';
}
